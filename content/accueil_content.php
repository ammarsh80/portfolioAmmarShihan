<main id="main_colonne_aacueil">
  <a href="index.php?page=cv_Ammar_Shihan">
    <img src="./assets/img/photo_profile.jpg" alt="Photo de profile"></a>
  <div id="my_header">
    <a href="index.php?page=cv_Ammar_Shihan">
      <h1>Ammar SHIHAN</h1>
      <h3 id="titre_dev">Développeur Web / Web Mobile</h3>
    </a>
  </div>
  <section id="acroche">
    <p> Doté d’une rigueur et d’une fiabilité scientifique,</p>
    <p>je me reconvertis dans le développement web
      pour persévérer en concevant, en réalisant
      et en pilotant des sites et des projets web</p>
    <br>
  </section>
</main>